namespace Application.Bids.Commands.CreateBid
{
    ...
    public class CreateBidCommandHandler : IRequestHandler<CreateBidCommand>
    {
        ...
        public async Task<Unit> Handle(CreateBidCommand request, CancellationToken cancellationToken)
        {
            ...
            var saldo = await this.context.SaldoUsers.FirstOrDefaultAsync(x => x.UserId == bid.UserId);
            if (saldo != null)
            {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var saldo = await this.context.SaldoUsers.FirstOrDefaultAsync(x => x.UserId == this.currentUserService.UserId);
            if (request.Amount > saldo.Saldo)
            {
                throw new BadRequestException(ExceptionMessages.Bid.InvalidBalanceAmount);
            }
        }
    }
}